...
${toSuggest}`)
    ...
    case "earthquake":
      ...
      slash.deferReply()
      request(
        `https://exptech.com.tw/api/v1/earthquake/reports?limit=${eqIndex}`,
        (error, response, body) => {
          const data = JSON.parse(body)[eqIndex - 1]
          const img = `https://www.cwb.gov.tw/Data/earthquake/img/${data.identifier.replace(
            "CWB-",
            ""
          )}.png`
          const locmap = `https://www.google.com/maps/search/?api=1&query=${data.epicenterLat},${data.epicenterLon}`
          const cwburl = `https://www.cwb.gov.tw/V8/C/E/EQ/${data.identifier
            .replace("CWB-", "")
            .replace(`${new Date().getFullYear()}-`, "")}.html`
          let biggestinte = []
          data.data.forEach((area) => {
            biggestinte.push(`${area.areaName}最大${area.areaIntensity}級`)
          })
          ...
          const eqEmbed = new builders.EmbedBuilder()
            .addFields({
              name: "編號",
              value:
                data.earthquakeNo != (new Date().getFullYear() - 1911) * 1000
                  ? data.earthquakeNo
                  : "無 (小區域有感地震)",
              inline: true,
            })
            .addFields({
              name: "時間",
              value: `<t:${new Date(data.originTime) / 1000}:d><t:${
                new Date(data.originTime) / 1000
              }:T>`,
              inline: true,
            })
            .addFields({ name: "深度", value: `${data.depth}km`, inline: true })
            .addFields({
              name: "芮氏規模",
              value: data.magnitudeValue.toString(),
              inline: true,
            })
            .addFields({
              name: "震央",
