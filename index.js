...
const game = new DiscordGame(process.env.TOKEN, 'youtube', 2, { neverExpire: false })
const helpRow = require('./help')
const hahalist = ['冰塊最想做什麼事?||退伍 因為他當冰很久了||', '有一天,我去吉野家,可是||吉野不在家||', '我走進眼科診所跟醫生抱怨說:「最近視力變差了,我需要配一副新眼鏡。」他||嘆了一口氣回說:「你真的病得不輕，我這裡可是甜甜圈店啊!」||', '有一隻狼寶寶不吃肉只吃素,狼媽媽、狼爸爸看得很擔心,某天,狼寶寶終於追著一隻兔子跑,牠們感到很欣慰,狼寶寶抓到兔子後說:||快把紅蘿蔔交出來!||', '天上的星星有多重?||8克,因為星巴克||', '有一天,小明去醫院量血壓,血壓計的語音說:血壓升高中，請注意...小明問醫生:為什麼會這樣?醫生回:這表示你的血壓...||國中畢業了。||', '第一個進船的要說什麼?||要說online,因為仙境傳說online||', '小魚問大魚說:你-喜-歡-吃-怎-樣-的-魚?大魚回:我喜歡吃講話慢的魚!小魚說:||醬紫先走||', '小明每次開可樂,瓶蓋都寫銘謝惠顧,有一天,他在考試,突然忘記銘要怎麼寫了,於是他打開桌上的可樂,||結果寫:再來一瓶||', '有一天,我和牛弟弟在吃草,弟弟問我:草是什麼味道?我回:草莓味。弟弟吃了一口草,生氣的說:這草明明沒有味道!我回:我沒有說錯啊...||我剛剛說草沒有味道,草沒味啊!||', '你知道學校的警衛每天早上都在笑什麼嗎?||校門口||', '什麼情況下人會有四隻眼睛?||兩個人的時候||', '愚公臨死前召集兒子來到床邊虛弱的說:移山…移山……\n兒子: ||亮晶晶?||\n愚公卒。']
const alphlist = ['🇦', '🇧', '🇨', '🇩', '🇪', '🇫', '🇬', '🇭', '🇮', '🇯', '🇰', '🇱', '🇲', '🇳', '🇴', '🇵', '🇶', '🇷', '🇸', '🇹', '🇺', '🇻', '🇼', '🇽', '🇾', '🇿']
...
process.on('uncaughtException', e => { console.log(`${chalk.magenta("哞!")} ${chalk.green("指令系統")}發生了${chalk.red("錯誤")}!\n${e}`) })
...
client.on('ready', async () => {
    console.clear()
    console.log(`${chalk.magenta("牛牛")} v0.3.3`)
    console.log(chalk.magenta(`
    █   █ ▀█▀ ▀█▀ █   █▀▀   █▀▀ █▀█ █ █ █
    █▄▄ █  █   █  █▄▄ ██▄   █▄▄ █▄█ ▀▄▀▄▀
    `))
    console.log(`${chalk.magenta("哞!")} ${chalk.green("指令系統")}已用 @${client.user.tag} 的身份登入!`)
    require('./music')
})

client.on('interactionCreate', async slash => {
    if (!slash.isCommand()) return
    const command = slash.options.getSubcommandGroup(false) || slash.options.getSubcommand()
    switch (command) {
        case 'help':
        ...
        case 'haha':
            ...
            slash.channel.send(hahalist[rnum(1, hahalist.length) - 1])
            break;
        case 'coin':
            slash.reply({ content: '哞!你要猜正面還是反面?', components: [coinButtonRow] })
            break;
        case 'say':
        ...
        case 'suggest':
            ...
            const suggestEmbed = new builders.EmbedBuilder()
                .setColor(0xffe153)
...
${toSuggest}`)
        ...
        case 'custompoll':
            ...
            const customPollOptions = slash.options.getString('option').split('@').join('​@')
            ...
            slash.channel.send(`${toCustomPoll}\n${pollopt.join('\n')}`).then(async (custompoll) => {
        ...
        case 'screenshot':
            ...
            if (!(toScreenshot.startsWith('http://') || toScreenshot.startsWith('https://'))) { return slash.reply({ content: '哞!這不是網址!', ephemeral: true }) }
            slash.deferReply()
        ...
        case 'shortlink':
            ...
            if (!(toShort.startsWith('http://') || toShort.startsWith('https://'))) return slash.reply({ content: '哞!這不是網址!', ephemeral: true })
            const datas = {
            ...
            request.post(datas, (err, res, body) => {
                if (err) {
                    return slash.editReply({ content: `:flushed: 哞!發生錯誤!`, ephemeral: true })
                }
                slash.editReply({ content: `哞!你的短網址: \`${JSON.parse(body)['url']}\``, ephemeral: false })
            })
            break;
        case 'botinfo':
            const rendermsg = ['牛牛 v0.3.3']
            ...
            break;
        case 'delay':
            slash.reply({ content: '哞!載入中...', fetchReply: true }).then(async (repli) => {
                slash.editReply(`哞!機器人延遲是:${repli.createdTimestamp - slash.createdTimestamp}ms | API延遲是:${client.ws.ping}ms`)
            })
            break;
        case 'minecraft':
            ...
            switch (minecraftType) {
                case 'bedrock':
                    request(`https://api.mcsrvstat.us/bedrock/2/${minecraftIp}`, (error, response, body) => {
                        body = JSON.parse(body)
                        if (!body.online) return slash.editReply({ content: '哞!伺服器沒開!' })
                        const dataembed = new builders.EmbedBuilder()
                            .addFields({ name: '名稱', value: body.motd.clean.join('\n') || "查無資料", inline: true })
                            ...
                            .addFields({ name: '模式', value: (body.gamemode || "查無資料"), inline: true })
                            .addFields({ name: '軟體', value: (body.software || "查無資料"), inline: true })
                            .addFields({ name: '端口', value: (body.port || "查無資料"), inline: true })
                        slash.editReply({ content: `哞!這是 \`${minecraftIp}\` 的資訊:`, embeds: [dataembed] })
                    })
                    break;
                case 'java':
                    request(`https://api.mcsrvstat.us/2/${minecraftIp}`, (error, response, body) => {
                        body = JSON.parse(body)
                        if (!body.online) return slash.editReply({ content: '哞!伺服器沒開!' })
                        const dataembed = new builders.EmbedBuilder()
        ...
        case 'youtube':
            ...
            if (toYoutube.type != 'GUILD_VOICE') return slash.reply({ content: '哞!這不是語音頻道!', ephemeral: true })
            game.play(toYoutube).then(result => slash.reply(`哞!點擊連結開始一起看YouTube! <${result.inviteLink}>`))
            break;
        case '2048':
        ...
        case 'invoice':
            slash.deferReply().then(() => {
                request('https://invoice.etax.nat.gov.tw/invoice.xml', (error, response, body) => {
                    inv = body
                    const opt = { ignoreComment: true, alwaysChildren: true }
                    const resu = xml.xml2js(inv, opt)
                    const title = resu.elements[0].elements[0].elements[4].elements[0].elements[0].cdata
                    const content = resu.elements[0].elements[0].elements[4].elements[3].elements[0].cdata.split('</p><p>')
                    content[0] = content[0].split('<p>')[1]
                    content[content.length - 1] = content[content.length - 1].split('</p>')[0]
                    const embed = new builders.EmbedBuilder()
                        .setTitle(title)
                        .addFields({ name: content[0].split('：')[0], value: content[0].split('：')[1] })
                        .addFields({ name: content[1].split('：')[0], value: content[1].split('：')[1] })
                        .addFields({ name: content[2].split('：')[0], value: content[2].split('：')[1].replaceAll('、', '\n') })
                    slash.editReply({ content: '哞!這是第1頁', embeds: [embed], components: [toggleButtonRow] })
                })
            })
            break;
        case 'color':
            ...
            if (parseInt(`0x${color}`).toString() == 'NaN') return slash.reply({ content: '哞!這不是色號!', ephemeral: true })
            let embed
            try {
        ...
        case 'lyrics':
            ...
            const slRow = new builders.ActionRowBuilder()
                .addComponents(searchLyrics)
            slash.reply({ content: '哞!點擊此按鈕即可打開搜尋表單!', components: [slRow] })
            break;
    }
})
client.on('interactionCreate', async button => {
    ...
    switch (button.customId) {
        case 'toggleButtonLeft':
            let nowPage = parseInt(button.message.content.split('哞!這是第')[1].split('頁')[0])
            button.deferUpdate()
            if (nowPage == 1) return
            request('https://invoice.etax.nat.gov.tw/invoice.xml', (error, response, body) => {
                inv = body
                const opt = { ignoreComment: true, alwaysChildren: true }
                const resu = xml.xml2js(inv, opt)
                const title = resu.elements[0].elements[0].elements[3 + nowPage - 1].elements[0].elements[0].cdata
                const content = resu.elements[0].elements[0].elements[3 + nowPage - 1].elements[3].elements[0].cdata.split('</p><p>')
                content[0] = content[0].split('<p>')[1]
                content[content.length - 1] = content[content.length - 1].split('</p>')[0]
                const embed = new builders.EmbedBuilder()
                    .setTitle(title)
                    ...
                    .addFields({ name: content[1].split('：')[0], value: content[1].split('：')[1] })
                    .addFields({ name: content[2].split('：')[0], value: content[2].split('：')[1].replaceAll('、', '\n') })
                button.message.edit({ content: `哞!這是第${nowPage - 1}頁`, embeds: [embed], components: [toggleButtonRow] })
            })
            break;
        case 'toggleButtonRight':
            let nowPageR = parseInt(button.message.content.split('哞!這是第')[1].split('頁')[0])
            button.deferUpdate()
            ...
            request('https://invoice.etax.nat.gov.tw/invoice.xml', (error, response, body) => {
                ...
                const opt = { ignoreComment: true, alwaysChildren: true }
                const resu = xml.xml2js(inv, opt)
                const title = resu.elements[0].elements[0].elements[3 + nowPageR + 1].elements[0].elements[0].cdata
                const content = resu.elements[0].elements[0].elements[3 + nowPageR + 1].elements[3].elements[0].cdata.split('</p><p>')
                content[0] = content[0].split('<p>')[1]
                content[content.length - 1] = content[content.length - 1].split('</p>')[0]
                const embed = new builders.EmbedBuilder()
                    .setTitle(title)
                    ...
                    .addFields({ name: content[1].split('：')[0], value: content[1].split('：')[1] })
                    .addFields({ name: content[2].split('：')[0], value: content[2].split('：')[1].replaceAll('、', '\n') })
                button.message.edit({ content: `哞!這是第${nowPageR + 1}頁`, embeds: [embed], components: [toggleButtonRow] })
            })
            break;
        case 'coinButtonHead':
        case 'coinButtonTail':
            ...
            if (coinnum == 1 && button.customId == 'coinButtonHead') return button.message.edit({ content: '答對了!', components: [] })
            if (coinnum == 1 && button.customId == 'coinButtonTail') return button.message.edit({ content: '錯了,是正面才對', components: [] })
            if (coinnum == 2 && button.customId == 'coinButtonHead') return button.message.edit({ content: '錯了,是反面才對', components: [] })
            if (coinnum == 2 && button.customId == 'coinButtonTail') return button.message.edit({ content: '答對了!', components: [] })
            break;
    }
})
