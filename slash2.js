...
s4d.client.ws.on('INTERACTION_CREATE', async interaction => {
    ...
    try {
        console.log(`slash beta user id:${interaction.member.user.id}`)
    }
    catch(err){
  ...
  if (interaction.data.name === 'cow-beta'){ 
 ...
 //#region help
    ...
    s4d.client.api.interactions(interaction.id, interaction.token).callback.post({
        data: {
            type: 4,
            data: {
                content: String((['哞!我是牛牛,一隻很簡單的機器牛。', '目前有下列功能:', '`/cow-beta haha` :讓我為你說一句笑話!', '`/cow-beta coin` :讓我陪你玩猜硬幣小遊戲!', '`/cow-beta say` :讓我一字不差的學你說話!', '`/cow-beta delay` :測測看我的延遲!', '`/cow-beta invite` :把我邀請到你的伺服器!', '`/cow-beta suggest` :告訴我你想到的新功能!', '`/cow-beta poll` :讓我為你舉行一場投票!', '`/cow-beta avatar` :好奇某人的頭貼長什麼樣子嗎?讓我來幫你看他的頭貼!', '`/cow-beta time` :看看現在的時間!', '`/cow-beta screenshot` :好奇某個網站什麼樣子嗎?讓我來幫你看它的樣子!', '`/cow-beta shortlink` :縮短你的網址!', '敬請期待更多功能!哞~'].join('\n')))
            }
        }
    })
 }
